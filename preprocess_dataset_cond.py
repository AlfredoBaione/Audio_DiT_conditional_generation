"""
preprocess_dataset_cond.py  (v3 - conditioned variant)

Preprocessing script for the CONDITIONED Audio DiT.

Identical to preprocess_dataset.py (unconditional) EXCEPT that it ALSO keeps
the train-split WAV files on disk (wav/train/...), instead of deleting them
after DAC encoding. The conditioned pipeline needs the real per-chunk audio of
EVERY split (train included) so that extract_conditions.py can extract the
frame-level conditions (melody / chroma / rhythm) from the ORIGINAL audio
rather than from a DAC-decoded reconstruction.

Condition extraction itself is NOT done here: it stays a separate, modular,
incremental step (extract_conditions.py), so adding a new condition later only
recomputes the missing .npz, not the whole dataset.

Pipeline:
    Raw audio -> Qualitative preprocessing -> Chunk -> DAC encode -> Latents .npy
    (train WAVs are kept, not deleted)

Output (structure identical to the unconditional dataset + train WAVs):
    output_dir/
        latents/ train|val|test / ClassName / ClassName_seg00001.npy
        wav/     train|val|test / ClassName / ClassName_seg00001.wav   <-- train INCLUDED
        metadata.csv
        class_mapping.json

Usage:
    python preprocess_dataset_cond.py source_dir output_dir --device cuda --max_workers 32
"""
...
def plan_and_assign_chunks(preprocessed_files: list, split_ratios: dict, seed: int) -> list:
    ...
    for pf in preprocessed_files:
        ...
        for i in range(n_chunks):
            ...
            # CONDITIONED: keep train WAVs too (under wav/train/...), so the
            # original per-chunk audio of every split survives for
            # extract_conditions.py. The unconditional script sent train chunks
            # to TEMP_DIR and deleted them after DAC encoding; here all splits
            # go to the persistent wav/ tree.
...
def encode_chunks_dac(saved_chunks: list, device: str, output_dir: str):
    ...
    for chunk in tqdm(saved_chunks, desc="Encoding DAC"):
        ...
        if latent_path.exists():
            # CONDITIONED: do NOT delete the train WAV here; all split WAVs are
            # kept on disk for condition extraction.
            n_ok += 1
            continue
        ...
        # CONDITIONED: train WAVs are intentionally NOT deleted (kept for
        # condition extraction from the original audio).

    del dac_model
    if torch.cuda.is_available():
        torch.cuda.empty_cache()

    print(f"\n[DAC] Done: {n_ok} OK, {n_err} errors")
...
def main():
    ...
    print(f"  Saved WAVs:     ALL splits (train + val + test) -- kept for condition extraction)")
    print(f"{'='*60}\n")
    ...
    print(f"  WAVs on disk:     {n_all_wav} (ALL splits, train included)")
    ...
    print(f"    {OUTPUT_DIR}/wav/train|val|test/<class>/*.wav")
